...
test.describe('Web Vitals Performance Tests', () => {
	...
	test.beforeEach(async ({ page }) => {
		...
		await page.addInitScript(() => {
			...
			import('https://unpkg.com/web-vitals@5/dist/web-vitals.attribution.js').then(({ getCLS, getFID, getFCP, getLCP, getTTFB }: any) => {
				// Collect all Core Web Vitals
				getCLS((metric: any) => (window as any).vitalsData.push({ name: 'CLS', value: metric.value, rating: metric.rating }));
				getFID((metric: any) => (window as any).vitalsData.push({ name: 'FID', value: metric.value, rating: metric.rating }));
				getFCP((metric: any) => (window as any).vitalsData.push({ name: 'FCP', value: metric.value, rating: metric.rating }));
				getLCP((metric: any) => (window as any).vitalsData.push({ name: 'LCP', value: metric.value, rating: metric.rating }));
				getTTFB((metric: any) => (window as any).vitalsData.push({ name: 'TTFB', value: metric.value, rating: metric.rating }));
			});
		});
	});
	...
	testPages.forEach(({ name, url }) => {
		test(`Web Vitals - ${name}`, async ({ page }) => {
			...
			// Get vitals data
			const vitals = await page.evaluate(() => (window as any).vitalsData || []) as VitalMetric[];
