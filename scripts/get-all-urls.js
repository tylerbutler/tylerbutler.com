...
async function getAllUrls(baseUrl = 'https://tylerbutler.com', newHost = null) {
  ...
  try {
        ...
        try {
          ...
          if (urlObj.hostname === baseUrlObj.hostname) {
            ...
            try {
              ...
              const hostWithProtocol = newHost.includes('://') ? newHost : `${urlObj.protocol}//${newHost}`;
              ...
              console.warn(`⚠️  Invalid newHost format: ${newHost}, skipping URL: ${url}`);
              return url;
            }
            rewrittenCount++;
    ...
    console.log(`✅ Complete! ${uniqueUrls.length} URLs saved to ${outputFile}`);

    if (newHost) {
      console.log(`🎯 Host rewritten from ${baseUrlObj.hostname} to ${newHost}`);
    }
