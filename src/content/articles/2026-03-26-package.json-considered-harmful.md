...
converts that text back into an in-memory object. That's it. That's the job.

Lots of languages have something like this. Python has [Pickle][] -- a binary format, notably not portable, not designed
...
dependencies -- all hand-authored. All human-maintained. And then there's the sprawl of tool configurations that have
...
No. And I'd argue they make it worse.
...
deleting every comment you've written. (I'm looking at you, [NX][].) Others support JSON but not JSONC, forcing you to
...
there the next time I open the file.
...
But if that question fills you with dread -- if you're thinking "but we need to *read* those files" -- then you're using
JSON as a human-readable format, and you should use a format designed for humans.
